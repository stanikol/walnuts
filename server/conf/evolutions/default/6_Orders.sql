...
--            o.tmpID, o.status, o.order_date as order_date
...
create view ordered_full_info as
    select gi.name, gi.price, gi.id as goods_item_id,
        ...
        order by email, order_id, ordered_item_id;
...
create view shop_cart_info as
    select gi.name, gi.price, gi.id as goods_item_id,
            sci.id as ordered_item_id, sci.qnt, sci.size, sci.total as item_total, sci.shop_cart_id
        from shop_cart_items sci left join goods_items gi on sci.goods_item_id = gi.id
        order by shop_cart_id, ordered_item_id;

# --- !Downs

drop view shop_cart_info;

drop table shop_cart_items;

drop view ordered_full_info;
