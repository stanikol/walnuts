...
Make sure the name has `.scss` extension (or `.sass` for Sass syntax). If you have just generated a new Rails app,
it may come with a `.css` name instead. If this name exists, it will be served instead of Sass, so rename it:

```console
$ mv app/assets/stylesheets/application.css app/assets/stylesheets/application.scss
```

Then, remove all the `*= require_self` and `*= require_tree .` statements from the sass name. Instead, use `@import` to import Sass files.
...
`bootstrap` provides a concatenated name containing all Bootstrap Javascripts.
...
* [styles.sass](/templates/project/styles.sass) - main project Sass name, imports Bootstrap and variables.
...
[`_bootstrap.scss`](assets/stylesheets/_bootstrap.scss) name into your assets as `_bootstrap-custom.scss`.
Then comment out components you do not want from `_bootstrap-custom`.
In the application Sass name, replace `@import 'bootstrap'` with:
...
Import Bootstrap into a Sass name (for example, `application.scss`) to get all of Bootstrap's styles, mixins and variables!
...
* Converts upstream Bootstrap LESS files to its matching SCSS name.
