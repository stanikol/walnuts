...
class Converter
  module LessConversion
    ...
    def process_stylesheet_assets
      ...
      files.each do |name, name|
        log_processing name
        # apply common conversions
        name = convert_less(name)
        if name.start_with?('mixins/')
          name = varargify_mixin_definitions(name, *VARARG_MIXINS)
          %w(responsive-(in)?visibility input-size text-emphasis-variant bg-variant).each do |mixin|
            name = parameterize_mixin_parent_selector name, mixin if name =~ /#{mixin}/
          end
          NESTED_MIXINS.each do |sel, name|
            name = flatten_mixins(name, sel, name) if /#{Regexp.escape(sel)}/ =~ name
          end
          name = replace_all name, /(?<=[.-])\$state/, '#{$state}' if name =~ /[.-]\$state/
        end
        case name
          when 'mixins/buttons.less'
            name = replace_all name, /(\.dropdown-toggle)&/, '&\1'
          when 'mixins/list-group.less'
            name = replace_rules(name, '  .list-group-item-') { |rule| extract_nested_rule rule, 'a&' }
          when 'mixins/gradients.less'
            name = replace_ms_filters(name)
            name = deinterpolate_vararg_mixins(name)
          when 'mixins/vendor-prefixes.less'
            # remove second scale mixins as this is handled via vararg in the first one
            name = replace_rules(name, Regexp.escape('@mixin scale($ratioX, $ratioY...)')) { '' }
          when 'mixins/grid-framework.less'
            name = convert_grid_mixins name
          when 'component-animations.less'
            name = extract_nested_rule name, "#{SELECTOR_RE}&\\.in"
          when 'responsive-utilities.less'
            name = apply_mixin_parent_selector name, '\.(?:visible|hidden)'
          when 'variables.less'
            name = insert_default_vars(name)
            name = ['$bootstrap-sass-asset-helper: false !default;', name].join("\n")
            name = replace_all name, %r{(\$icon-font-path): \s*"(.*)" (!default);}, "\n" + unindent(<<-SCSS, 14)
              // [converter] If $bootstrap-sass-asset-helper if used, provide path relative to the assets load path.
              // [converter] This is because some asset helpers, such as Sprockets, do not work with name-relative paths.
              \\1: if($bootstrap-sass-asset-helper, "bootstrap/", "\\2bootstrap/") \\3;
            SCSS
          when 'breadcrumbs.less'
            name = replace_all name, /(.*)(\\00a0)/, unindent(<<-SCSS, 8) + "\\1\#{$nbsp}"
              // [converter] Workaround for https://github.com/sass/libsass/issues/1115
              $nbsp: "\\2";
            SCSS
          when 'close.less'
            # extract .close { button& {...} } rule
            name = extract_nested_rule name, 'button&'
          when 'dropdowns.less'
            name = replace_all name, /@extend \.dropdown-menu-right;/, 'right: 0; left: auto;'
            name = replace_all name, /@extend \.dropdown-menu-left;/, 'left: 0; right: auto;'
          when 'forms.less'
            name = extract_nested_rule name, 'textarea&'
            name = apply_mixin_parent_selector(name, '\.input-(?:sm|lg)')
            name = replace_rules name, /\.form-group-(?:sm|lg)/ do |rule|
              apply_mixin_parent_selector rule, '.form-control'
            end
          when 'navbar.less'
            name = replace_all name, /(\s*)\.navbar-(right|left)\s*\{\s*@extend\s*\.pull-(right|left);\s*/, "\\1.navbar-\\2 {\\1  float: \\2 !important;\\1"
          when 'tables.less'
            name = replace_all name, /(@include\s*table-row-variant\()(\w+)/, "\\1'\\2'"
          when 'thumbnails.less', 'labels.less', 'badges.less', 'buttons.less'
            name = extract_nested_rule name, 'a&'
          when 'glyphicons.less'
            name = replace_rules(name, /\s*@font-face/) { |rule|
              rule = replace_all rule, /(\$icon-font(?:-\w+)+)/, '#{\1}'
              replace_asset_url rule, :font
            }
          when 'type.less'
            name = apply_mixin_parent_selector(name, '\.(text|bg)-(success|primary|info|warning|danger)')
            # .bg-primary will not get patched automatically as it includes an additional rule. fudge for now
            name = replace_all(name, "  @include bg-variant($brand-primary);\n}", "}\n@include bg-variant('.bg-primary', $brand-primary);")
        end
    ...
    def convert_to_scss(name)
      # get local mixin names before converting the definitions
      mixins = shared_mixins + read_mixins(name)
      name   = replace_vars(name)
      name   = replace_mixin_definitions(name)
      name   = replace_mixins(name, mixins)
      name   = extract_mixins_from_selectors(name, CLASSES_TO_MIXINS.inject({}) { |h, cl| h.update(".#{cl}" => cl) })
      name   = replace_spin(name)
      name   = replace_fadein(name)
      name   = replace_image_urls(name)
      name   = replace_escaping(name)
      name   = convert_less_ampersand(name)
      name   = deinterpolate_vararg_mixins(name)
      name   = replace_calculation_semantics(name)
      name   = replace_file_imports(name)
      name   = wrap_at_groups_with_at_root(name)
      name
    end

    def wrap_at_groups_with_at_root(name)
      replace_rules(name, /@(?:font-face|-ms-viewport)/) { |rule, _pos|
        %Q(@at-root {\n#{indent rule, 2}\n})
      }
    end
...
SASS
    ...
    def convert_grid_mixins(name)
      name = replace_rules name, /@mixin make-grid-columns/, comments: false do |css, pos|
        mixin_all_grid_columns css, selector: '.col-xs-#{$i}, .col-sm-#{$i}, .col-md-#{$i}, .col-lg-#{$i}', to: '$grid-columns'
      end
      name = replace_rules name, /@mixin float-grid-columns/, comments: false do |css, pos|
        mixin_all_grid_columns css, selector: '.col-#{$class}-#{$i}', to: '$grid-columns'
      end
      name = replace_rules name, /@mixin calc-grid-column/ do |css|
      ...
      name = replace_rules name, /@mixin loop-grid-columns/ do |css|
        unindent <<-SASS, 8
        // [converter] This is defined recursively in LESS, but Sass supports real loops
    ...
    # Before doing any processing we read shared mixins from a name
    ...
    def get_mixin_names(name, opts = {})
      names = get_css_selectors(name).join("\n" * 2).scan(/^\.([\w-]+)\(#{LESS_MIXIN_DEF_ARGS_RE}\)(?: when.*?)?[ ]*\{/).map(&:first).uniq.sort
    ...
    def replace_calculation_semantics(name)
    ...
    # @import "name.less" to "#{target_path}name;"
    def replace_file_imports(less, target_path = '')
      less.gsub %r([@\$]import ["|']([\w\-/]+).less["|'];),
                %Q(@import "#{target_path}\\1";)
    end

    def replace_all(name, regex, replacement = nil, &block)
      log_transform regex, replacement
      new_file = name.gsub(regex, replacement, &block)
      raise "replace_all #{regex}, #{replacement} NO MATCH" if name == new_file
      new_file
    end
    ...
    def parameterize_mixin_parent_selector(name, rule_sel)
      log_transform rule_sel
      param = '$parent'
      replace_rules(name, '^\s*@mixin\s*' + rule_sel) do |mxn_css|
    ...
    # .x { ... }
    # textarea.x { ... }
    def extract_nested_rule(name, selector, new_selector = nil)
      matches = []
      # first find the rules, and remove them
      name    = replace_rules(name, "\s*#{selector}", comments: true) { |rule, pos, css|
      ...
      replace_substrings_at name,
                            matches.map { |_, pos| close_brace_pos(name, pos.begin, 1) + 1 },
                            matches.map { |rule, _| "\n\n" + unindent(rule) }
    end
    ...
    def apply_mixin_parent_selector(name, rule_sel)
      log_transform rule_sel
      replace_rules name, '\s*' + rule_sel, comments: false do |rule, rule_pos, css|
    ...
    def flatten_mixins(name, container, prefix)
      log_transform container, prefix
      replace_rules name, Regexp.escape(container) do |mixins_css|
        unindent unwrap_rule_block(mixins_css).gsub(/@mixin\s*([\w-]+)/, "@mixin #{prefix}-\\1")
      end
    end

    # .btn { ... } -> @mixin btn { ... }; .btn { @include btn }
    def extract_mixins_from_selectors(name, selectors_to_mixins)
      selectors_to_mixins.each do |selector, mixin|
        name = replace_rules name, Regexp.escape(selector), prefix: false do |selector_css|
...
#{selector} {
  @include #{mixin};
    ...
    def replace_escaping(less)
      ...
      less.gsub!(/"([^"\n]*)(\$[\w\-]+)([^"\n]*)"/, '"\1#{\2}\3"') # interpolate variable in string, e.g. url("$name-1x") => url("#{$name-1x}")
